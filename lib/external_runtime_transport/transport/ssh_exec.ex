defmodule ExternalRuntimeTransport.Transport.SSHExec do
  ...
  alias ExecutionPlane.Process.Transport.SSHExec, as: RuntimeSSHExec
  ...
  @impl Adapter
  def capabilities do
    RuntimeSSHExec.capabilities()
    |> Map.from_struct()
    |> Capabilities.new!()
  end

  @impl Adapter
  defdelegate normalize_transport_options(options), to: RuntimeSSHExec
